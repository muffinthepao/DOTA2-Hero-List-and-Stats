...
class Heroes {
    ...
    updateChart(option) {
        const filteredByRank = this.arrayOfHeroes.map((hero) => {
            return {
                x: hero[`${option}Picks`],
...
async function init() {
    ...
    filterCharts.onchange = function (event) {
        const selectedOption = event.target.value;
        dota2Heroes.updateChart(selectedOption);
    };
...
// let picks = [];
            ...
            //     if (key.indexOf("_pick") > -1) {
            //         picks.push(key);
            //     } else if (key.indexOf("_win") > -1) {
            //         wins.push(key);
            //     }
            // });

            // picks = picks.map((key) => hero[key]);
            // wins = wins.map((key) => hero[key]);
